...
class FastSessionManager:
    ...
    def refresh_credentials(self):
        print("⚡ Fast-launching Chromium to capture session headers...")
        captured = {"headers": {}}
        ...
        try:
            with sync_playwright() as p:
                ...
                # Block heavy assets (images, stylesheets, fonts) to load in ~2 seconds
                ...
                try:
                    page.goto(self.root_url, wait_until="domcontentloaded", timeout=25000)
                    # Wait only until headers are captured (or max 5 seconds)
                    auth_event.wait(timeout=5)
                except Exception as e:
                    print(f"Page notice: {e}")
            ...
            if captured["headers"]:
                ...
                print("✅ Session credentials captured in record time.")
            else:
                print("⚠️ Retrying fallback headers...")
        except Exception as e:
            print(f"❌ Auth error: {e}")

    def ensure_fresh(self):
        if time.time() - self.last_auth_time > 1200 or not self.session.headers:
            self.refresh_credentials()

session_mgr = FastSessionManager(ROOT_URL)

def fetch_folder_with_backoff(session_mgr, folder_id, page_num=1, max_retries=3):
...
def crawl_gofile_recursive():
    ...
    with crawl_lock:
        if cached_videos and (time.time() - last_scan_time < 2700):
        ...
        if total_list:
            ...
            print(f"✅ Crawl complete: {len(cached_videos)} total playable files ready.")

        return cached_videos

def auto_cache_worker():
    """Keeps the catalog updated in the background so users never experience delays."""
    while True:
        try:
            time.sleep(2400)  # Refresh every 40 minutes
            crawl_gofile_recursive()
        except Exception as e:
            print(f"Background worker notice: {e}")

@asynccontextmanager
async def lifespan(app: FastAPI):
    threading.Thread(target=crawl_gofile_recursive, daemon=True).start()
    threading.Thread(target=auto_cache_worker, daemon=True).start()
    yield
...
def proxy_video_stream(item_id: str, request: Request):
    ...
    try:
        ...
        # 1MB buffer chunks for fast throughput
        def stream_generator():
            for chunk in upstream_res.iter_content(chunk_size=1024 * 1024):
                if chunk:
                    yield chunk
